...
const loginUser = async function(req,res){
  ...
  let login = await user1Model.findOne({emailId:email, password:password})
  if(!login){
...
//   console.log({msg: token})
  // if(!xtoken){
  //   return res.send({msg:"no match"})
  // }
  // let decodeToken = jwt.verify(xtoken ,"subbmit assignment")
  // if(!decodeToken){
  //   return res.send({msg: "invalid User"})
  // }
  // let userId = req.params.userId
  // let userDetails = await user1Model.findById(userId)
  // if(!userDetails){
...
const user = async function(req,res){
  let details = req.params.userId
  let userdetails = await user1Model.findById(details)
